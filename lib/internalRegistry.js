...
var InternalRegistry = function(config, db, targz, fs, crypto) { 
  ...
  var getModulePath = function(index, name, version) {
    return config.internal.tgz + '/' + index + '/' + name + '/' + version;
  };
  
  var getModuleVersionPath = function(index, name, version, callback) {
    var moduleVersionPath = getModulePath(index, name, version);
    mkdirp(moduleVersionPath, function() {
      callback(moduleVersionPath);
    });
  };
  ...
  var addTarball = function(index, name, version, path, callback) {
    getModuleVersionPath(name, name, version, function(modulePath) {
  ...
  var getModule = function(index, name, version, callback) {
    var path = getModulePath(index, name, version);
