...
var RegistryManager = function(config, registryCache, httpUtil, internalRegistry) {
  ...
  var getModuleIndexRecursive = function(whichOne, name, etag, callback) {
    ...
    log.debug('Looking for ' + name + ' (etag: ' + etag + ') on ' + currentRegistry.getRoot());
    currentRegistry.getModuleIndex(name, etag, function(err, json) {
      if (json === 304) {
        log.debug('Items Etag is the same (' + etag + '), item has not been modified');
        callback(undefined, null);
      } else if (err !==undefined || json === null) {
        getModuleIndexRecursive(whichOne+1, name, etag, callback);
      } else {
        log.debug('Found ' + name + ' in ' + currentRegistry.getRoot());
        if(json.etag !== etag && etag) {
          log.debug('Items ETag has been changed (was: ' + etag + ', now: ' + json.etag + '), cache will be updated');
        }
        // Cache it
    ...
    internalRegistry.getModuleIndex(name, function(err, item) {
      if (item) {
        ...
        registryCache.getIndex(name, function(err, item) {
          if (item) {
            getModuleIndexRecursive(0, name, item.etag, function(err, json) {
              callback(undefined, json || item);
            });
          } else {
            getModuleIndexRecursive(0, name, null, callback);
          }
        });
      }      
    });
