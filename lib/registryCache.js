...
var RegistryCache = function(config, db) {
  ...
  var validateIndex = function(name, item, callback) {
    if (!item) {
      log.info(name + ' was not found in the cache');
      callback(undefined, null);
    } else {
      if(item.dateAdded < getExpiryDate()) {
        log.info(name + ' has expired and will be removed from cache.');
        removeIndex(name, callback);
      } else {
        log.info(name + ' was found in the cache');
  ...
  var setIndex = function(jsonContents, callback) {
    indexCollection.insert({
      ...
      if (err) {
        ...
        log.info(jsonContents._id + ' was added to the cache');
  ...
  var removeIndex = function(name, callback) {
    log.info(name + ' will be removed from the cache.');
