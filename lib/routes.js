'use strict';
var Routes = function(config, registryManager, fs) {
  fs = fs || require('fs');
  ...
  function getPackageIndex(name) {
    ...
    registryManager.getModuleIndex(name, function(err, json) {
      ...
      if (statusCode === 404) {
        sendResponse(_this.res, statusCode);
      } else {
  ...
  function putPackage(name) {
    log.debug(name + ': Put Package');
    sendResponse(this.res, 200, {response: 'Module Indexed'});
  }
  var router = new director.http.Router();
  // /package
  router.get(/([a-zA-Z0-9_-]+)/, getPackageIndex);
  router.put(/([a-zA-Z0-9_-]+)/, putPackage);
